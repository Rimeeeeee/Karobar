...
const InsuranceCardb: React.FC<InsuranceSchemeProps> = ({
  pid,
  name,
  description,
  coverage,
  ...
  const handleBuyInsurance = async () => {
    ...
    const transaction = await prepareContractCall({
      ...
      params: [BigInt(pid)],
      value: ethers.parseEther(String(Number(deposit_amount_monthwise) / 1e18)),
    })
  ...
  return (
    <div className="bg-gradient-to-r from-black via-gray-900 to-black border-2 border-gray-600 rounded-xl shadow-lg transition-shadow duration-500 p-6 mb-6 max-w-md mx-auto ">
      ...
      <button
        ...
        className="w-full py-2 mt-3 bg-gradient-to-r from-purple-600 to-blue-500 text-white font-bold text-base rounded-lg shadow-lg hover:shadow-2xl hover:from-purple-700 hover:to-blue-600 transition-all duration-300 ease-in-out"
      >
        Request Money
      </button>
    </div>
